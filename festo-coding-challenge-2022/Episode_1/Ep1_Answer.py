...
if __name__ == "__main__":
    ...
    # puzzle 1
    filtered = filter_pico(read_lab(base_path / "data/lab_blood_clean.txt"))
    print(f"Bots found in clean samples: {len(filtered)}")
    filtered = filter_pico(read_lab(base_path / "data/lab_blood_gen1.txt"))
    print(f"Bots found in gen1 samples: {len(filtered)}")

    population = read_population(base_path / "data/population.txt")
    ...
    # puzzle 2
    galaxy = read_galaxy(base_path / "data/galaxy_map.txt")
    ...
    # puzzle 3
    log = read_security_log(base_path / "data/security_log.txt")
